...
def quiz(message):
    ...
    res = getChat(f"""یک کوییز یک سوالی و 4 جوابی از مبحث {context}و با سطح سختی {level}  از 10 طراحی کن و فرمت سوالات و پاسخ را به شکل زیر ارسال کن مطلقا بدون هیچ توضیحات اضافه ای و در جاهایی مشخص شده قرار بده همچنین هیچ متی را Bold نکن یا هیچ استایلی روی ان قرار نده و از seed رندوم {getId()} استفاده کن تا سوال تکراری تولید نکنی
فرمت مد نظر:
صورت سوال:
<صورت سوال>
1.<گزینه اول>
